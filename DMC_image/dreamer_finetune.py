...
class Workspace:
    def __init__(self, cfg, savedir=None, workdir=None):
        ...
        self.frame_stack = 1
        self.img_size = 64
        self.train_envs = [envs.make(task, cfg.obs_type, self.frame_stack,
                                     cfg.action_repeat, cfg.seed, img_size=self.img_size)
                           for task in tasks]
        ...
        if cfg.task != 'none':
            ...
            # pre-define multi-task
            # print(FINETUNE_TASKS.keys())
            if (self.cfg.finetune_domain + '_eval') in FINETUNE_TASKS.keys():
                eval_tasks = FINETUNE_TASKS[self.cfg.finetune_domain + '_eval']
            else:
                eval_tasks = []
        self.eval_envs = [envs.make(task, cfg.obs_type, self.frame_stack,
                                    cfg.action_repeat, cfg.seed, img_size=self.img_size)
        ...
        # create agent 
        exceptions = ['diayn_dreamer', 'aps_dreamer', 'lsd_dreamer', 'cic_dreamer', 'choreo'] # skill approaches
        ...
        elif (('peac' in cfg.agent.name) or ('context' in cfg.agent.name)) and cfg.init_task > 0.0:
            print('we utilize peac')
            self.agent = make_dreamer_agent(self.train_envs[0].obs_space,
                                            self.train_envs[max_act_index].action_spec(),
                                            cfg, cfg.agent)
            self.agent.finetune_mode()
        else:
            self.agent = DreamerAgent(cfg.agent.name, cfg, obs_space,
                                      self.train_envs[max_act_index].action_spec())
    ...
    def eval(self):
        ...
        train_envs = [envs.make(task, self.cfg.obs_type, self.frame_stack,
        ...
        for eval_id in range(self.eval_envs_number):
            ...
            while eval_until_episode(current_episode):
                dreamer_obs = self.eval_envs[eval_id].reset()
                dreamer_obs['embodiment_id'] = self.train_envs_number + eval_id
                data = dreamer_obs
                agent_state = None
                if self.cfg.save_eval_episodes:
                    self.eval_storage.add(data, meta,
                                          env_id=self.train_envs_number + eval_id)
                while not bool(dreamer_obs['is_last']):
                    with torch.no_grad(), utils.eval_mode(self.agent):
                        if self.cfg.mpc:
                            ...
                            action, agent_state = self.agent.act(dreamer_obs,
                                                    ...
                                                    eval_mode=True,
                                                    state=agent_state)
                    dreamer_obs = self.eval_envs[eval_id].step(action)
                    dreamer_obs['embodiment_id'] = self.train_envs_number + eval_id
                    data = dreamer_obs
                    if self.cfg.save_eval_episodes:
                        self.eval_storage.add(data, meta,
                    ...
                    eval_reward += dreamer_obs['reward']
                    step += 1
                    if not bool(dreamer_obs['is_last']):
                        meta = self.agent.update_meta(meta, step, dreamer_obs)
        ...
        for train_id in range(self.train_envs_number):
            ...
            current_rews = []
            while eval_until_episode(current_episode):
                dreamer_obs = train_envs[train_id].reset()
                dreamer_obs['embodiment_id'] = train_id
                data = dreamer_obs
                agent_state = None
                if self.cfg.save_eval_episodes:
                    self.eval_storage.add(data, meta,
                ...
                while not bool(dreamer_obs['is_last']):
                    with torch.no_grad(), utils.eval_mode(self.agent):
                        if self.cfg.mpc:
                            ...
                            action, agent_state = self.agent.act(dreamer_obs,
                                                    ...
                                                    eval_mode=True,
                                                    state=agent_state)
                    dreamer_obs = train_envs[train_id].step(action)
                    dreamer_obs['embodiment_id'] = train_id
                    data = dreamer_obs
                    if self.cfg.save_eval_episodes:
                        self.eval_storage.add(data, meta,
                    ...
                    total_reward += dreamer_obs['reward']
                    ep_rew += dreamer_obs['reward']
                    train_reward += dreamer_obs['reward']
                    step += 1
                    if not bool(dreamer_obs['is_last']):
                        meta = self.agent.update_meta(meta, step, dreamer_obs)
        ...
        while train_until_step(self.global_step):
            if bool(dreamer_obs['is_last']):
                ...
                if metrics is not None:
                    ...
                    print('Sampled env:', self.train_tasks_name[self.current_train_id])
