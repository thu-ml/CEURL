...
class LBS_ContextAgent(DDPGAgent):
    def __init__(self, lbs_scale, update_encoder,
        ...
        self.task_model = TaskModel(self.obs_dim, self.action_dim, self.context_dim,
    ...
    def update_task_model(self, obs, action, next_obs, task_id, pre_obs, pre_acts):
        ...
        loss = F.cross_entropy(task_pred, task_id.reshape(-1))
    ...
    def update_lbs(self, obs, action, next_obs, task_id, pre_obs, pre_acts):
        metrics = dict()
        ...
        with torch.no_grad():
            context_pred = F.softmax(self.task_model(pre_obs, pre_acts), dim=-1)

        kl_pred = self.lbs_pred(context_pred, obs, action, next_obs)
    ...
    def update(self, replay_iter, step):
        ...
        if self.reward_free:
            ...
            metrics.update(self.update_lbs(obs, action, next_obs, task_id,
                                           his_o, his_a))
