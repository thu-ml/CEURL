...
class RMS(object):
    ...
    def __call__(self, x):
        ...
        new_S = (self.S * self.n + torch.var(x, dim=0) * bs + (delta ** 2) * self.n * bs / (self.n + bs)) / (
                    self.n + bs)
...
def compute_apt_reward(source, target, args, device, rms):
...
class CICAgent(DDPGAgent):
    ...
    def __init__(self, update_skill_every_step, skill_dim, scale,
        ...
        self.cic = CIC(self.obs_dim - skill_dim, skill_dim,
                       kwargs['hidden_dim'], project_skill).to(self.device)
    ...
    def compute_apt_reward(self, obs, next_obs):
        ...
        reward = compute_apt_reward(source, target, args, device=self.device,
                                    rms=self.rms)  # (b,)
    ...
    def update(self, replay_iter, step):
        ...
        obs, action, extr_reward, discount, next_obs, task_id, skill = utils.to_torch(
            batch, self.device)
